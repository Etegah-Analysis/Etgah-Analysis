...
const Dashboard = () => {
  ...
  const [subReceiptDate, setSubReceiptDate] = useState('');
          ...
          <button
            ...
            className={`relative group p-0.5 rounded-lg transition-all duration-200 cursor-pointer shadow-[0_2px_8px_rgba(0,0,0,0.25)] hover:shadow-[0_4px_12px_rgba(245,158,11,0.4)] hover:scale-110 active:scale-95 border ${
              isFiltered
                ...
                : 'bg-gradient-to-br from-slate-900 via-amber-950/80 to-slate-900 hover:from-amber-950 hover:to-orange-950 border-amber-500/40 md:hover:border-amber-300'
            }`}
...
// Edit Employee Modal
  ...
  const handleCardClick = (e, type, filter = 'all') => {
    ...
    // Batch all updates into a single atomic render pass for maximum speed
    if (type === 'leads_crm') {
...
🕒 وقت التسجيل: ${item.uploadedDateTime || new Date().toLocaleString('ar-EG')}`;
  ...
  const handleSaveSubscriptionDetails = async (e) => {
    ...
    if (!subReceiptDate) {
      toast.error('يرجى تحديد تاريخ الإشعار / التحويل (حقل إجباري) ⚠️');
      return;
    }
    if (!subStartDate) {
    ...
    if (!subReceiptFileUrl) {
      toast.error('يرجى إرفاق صورة أو ملف إشعار التحويل (حقل إجباري) ⚠️');
      return;
    }
              ...
              <div className="relative shrink-0 z-[1000]" ref={notifDropdownRef}>
                <button 
                  ...
                  className={`group/bell relative flex items-center justify-center px-3 py-1.5 rounded-2xl transition-all duration-300 font-bold gap-2 cursor-pointer active:scale-95 border ${
                    isBellRed 
                      ? 'bg-gradient-to-b from-red-600 via-rose-600 to-red-700 text-white border-red-300 shadow-[0_4px_16px_rgba(225,29,72,0.65),inset_0_1px_2px_rgba(255,255,255,0.6)] animate-pulse hover:shadow-[0_6px_22px_rgba(225,29,72,0.9)] md:hover:-translate-y-0.5' 
                      : 'bg-white/10 hover:bg-white/20 text-amber-200 border-white/20 shadow-[0_2px_8px_rgba(0,0,0,0.3)] md:hover:-translate-y-0.5'
                  }`}
                ...
                <div className="px-6 py-3.5 bg-gradient-to-r from-purple-50/70 via-indigo-50/40 to-white border-b flex flex-wrap justify-between items-center gap-3">
                  ...
                  {isAdmin && !isLeader && selectedLeadsCrm.length > 0 && (
                    <button 
                      ...
                      className="bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-full text-xs font-black transition flex items-center gap-1.5 shadow-md transform md:hover:scale-105 active:scale-95"
                    >
                                ...
                                <td className="p-3.5 text-center font-mono font-bold text-gray-900" dir="ltr">
                                  <div className="flex items-center justify-center gap-1.5 flex-wrap">
                                    ...
                                    {customer.phoneNumber && !isCoordinator && (
                                      <button
                                        onClick={(e) => { e.stopPropagation(); handleCallViaMicroSip(customer.phoneNumber); }}
                                        className="bg-gradient-to-r from-blue-600 via-indigo-600 to-cyan-500 hover:from-blue-500 hover:to-cyan-400 text-white shadow-[0_3px_10px_rgba(37,99,235,0.4)] hover:shadow-[0_5px_15px_rgba(37,99,235,0.6)] active:scale-95 border border-blue-300/40 rounded-lg px-2 py-0.5 text-[11px] font-black flex items-center gap-1 cursor-pointer transform md:hover:-translate-y-0.5 transition-all shrink-0"
                                        title="اتصال مباشر عبر MicroSIP 📞"
                                      >
                                        <PhoneCall size={12} className="animate-pulse" />
                                        <span>Call</span>
                        ...
                        <div className="flex items-center gap-2">
                          {!isCoordinator && visitor.phone && (
                            <button
                              ...
                              className="bg-gradient-to-r from-blue-600 via-indigo-600 to-cyan-500 hover:from-blue-500 hover:to-cyan-400 text-white shadow-[0_3px_10px_rgba(37,99,235,0.4)] hover:shadow-[0_5px_15px_rgba(37,99,235,0.6)] active:scale-95 border border-blue-300/40 rounded-lg px-2.5 py-0.5 text-[11px] font-black flex items-center gap-1 cursor-pointer transform md:hover:-translate-y-0.5 transition-all shrink-0"
                              title="اتصال مباشر عبر MicroSIP 📞"
                            >
                              <PhoneCall size={12} className="animate-pulse" />
                              <span>Call</span>
                      ...
                      <div className="space-y-6">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                          <div 
                            ...
                            className="bg-gradient-to-br from-indigo-950 via-purple-950 to-slate-900 p-4 rounded-2xl border border-amber-400/50 md:hover:border-amber-300 md:hover:scale-[1.02] transition-all cursor-pointer shadow-lg group flex items-center justify-between"
        ...
        {isSubscriptionModalOpen && selectedSubCustomer && typeof document !== 'undefined' && document.body && createPortal(
          ...
          >
            ...
            <div className="bg-slate-900 text-white rounded-3xl shadow-2xl w-full max-w-xl p-6 relative z-10 max-h-[84vh] my-auto flex flex-col border border-emerald-500/40 overflow-hidden cursor-default" onClick={(e) => e.stopPropagation()}>
              ...
              {/* Modal Body / Form */}
              <form onSubmit={handleSaveSubscriptionDetails} className="flex-1 min-h-0 flex flex-col overflow-hidden">
                <div ref={modalFormRef} className="flex-1 overflow-y-auto space-y-4 pr-1 custom-scroll pb-2">
                <div ref={modalTopRef} />
                ...
                {/* Dynamic Service & Receipt Dates */}
                <div className="bg-slate-950/60 p-3.5 rounded-2xl border border-emerald-500/20 space-y-3">
                  {/* Receipt Date - فصلة تاريخ الإشعار */}
                  <div>
                    <label className="block text-xs font-bold text-cyan-200 mb-1.5 flex items-center justify-between">
                      <span className="flex items-center gap-1">
                        <span>🧾 تاريخ الإشعار / التحويل</span>
                        <span className="text-rose-400 font-black">*</span>
                      </span>
                      <span className="text-[10px] text-cyan-300 font-normal">تاريخ سداد هذه الدفعة</span>
                    </label>
                    <input 
                      type="date"
                      required
                      ...
                      onClick={(e) => { try { e.target.showPicker(); } catch (err) {} }}
                      className={`w-full px-3 py-2 bg-slate-900 border border-cyan-500/40 rounded-xl text-xs font-bold outline-none focus:border-cyan-400 cursor-pointer text-center date-input-dark ${!subReceiptDate ? 'text-transparent empty-date' : 'text-white'}`}
                      dir="ltr"
                    />
                  </div>

                  {/* Service Start & End Dates */}
                  <div className={`grid grid-cols-1 ${(subServiceType === 'اتفاق نسبة' || subPaymentType === 'percentage') ? 'sm:grid-cols-1' : 'sm:grid-cols-2'} gap-3 pt-2.5 border-t border-slate-800/80`}>
                    <div>
...
{/* Receipt Code input removed as requested */}
                </div>
                ...
                {/* Sticky Bottom Action Bar - دائماً ظاهر وثابت بالأسفل لإجبار الحفظ والإغلاق */}
                <div className="shrink-0 pt-3 pb-1 border-t border-slate-800 bg-slate-900/95 flex justify-end items-center gap-3 z-20">
                  <button
                    type="button"
                    onClick={() => setIsSubscriptionModalOpen(false)}
                    className="px-4 py-2 rounded-xl text-xs font-bold bg-slate-800 hover:bg-slate-700 text-slate-300 transition cursor-pointer active:scale-95"
                  >
                    إلغاء
                  </button>
                  ...
                  <button
                    type="submit"
                    disabled={subSaving}
                    className={`py-2.5 px-6 rounded-xl text-xs font-black text-white shadow-lg transition flex items-center gap-2 cursor-pointer disabled:opacity-50 active:scale-95 ${editingReceiptId ? 'bg-gradient-to-r from-amber-600 to-orange-600 hover:from-amber-500 hover:to-orange-500 shadow-[0_4px_15px_rgba(245,158,11,0.4)]' : 'bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-700 hover:to-teal-700 shadow-[0_4px_15px_rgba(16,185,129,0.4)]'}`}
                  >
                    <Save size={15} />
