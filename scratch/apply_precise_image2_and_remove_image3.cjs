...
// 1. Remove Image 3 paste box from AddBuffetItemModal
const itemPasteLabel = '<label className="block text-xs font-bold text-emerald-300 mb-1">📋 مكان للصق نص أو صورة / سكرين شوت للصنف (اختياري)</label>';
const itemPasteIdx = content.indexOf(itemPasteLabel);
if (itemPasteIdx !== -1) {
  // Find container wrapper <div> before label and closing </div> after input
  const startDiv = content.lastIndexOf('<div>', itemPasteIdx);
  const endInput = content.indexOf('/>\n                  </div>\n                </div>', itemPasteIdx);
  if (startDiv !== -1 && endInput !== -1) {
    const fullBlock = content.slice(startDiv, endInput + '/>\n                  </div>\n                </div>'.length);
    content = content.replace(fullBlock, '');
    console.log('Successfully removed paste box from AddBuffetItemModal!');
  }
}

// 2. Remove Image 3 paste box from AddBuffetPurchaseModal
const purchPasteLabel = '<label className="block text-xs font-bold text-blue-300 mb-1">📋 مكان للصق نص أو صورة / سكرين شوت للفاتورة (اختياري)</label>';
const purchPasteIdx = content.indexOf(purchPasteLabel);
if (purchPasteIdx !== -1) {
  const startDiv = content.lastIndexOf('<div>', purchPasteIdx);
  const endInput = content.indexOf('/>\n                  </div>\n                </div>', purchPasteIdx);
  if (startDiv !== -1 && endInput !== -1) {
    const fullBlock = content.slice(startDiv, endInput + '/>\n                  </div>\n                </div>'.length);
    content = content.replace(fullBlock, '');
    console.log('Successfully removed paste box from AddBuffetPurchaseModal!');
  }
}

// 3. Replace Buffet tab layout to match Image 2
